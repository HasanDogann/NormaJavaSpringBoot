...
@Slf4j
public class CardPaymentConverterImpl implements CardPaymentConverter {

    private final CardRepository cardRepository;

    @Override
    public Card toCardFromATMPaymentDTO(CardPaymentRequestDTO cardPaymentRequestDTO) {

        Card card = cardRepository.getCardByCardNo(cardPaymentRequestDTO.cardNo());

        if (cardPaymentRequestDTO.paymentType().equals(PaymentType.BY_ATM)) {
    ...
    @Override
    public Card toCardFromAccountPaymentDTO(CardPaymentRequestDTO cardPaymentRequestDTO) {
        Card card = cardRepository.getCardByCardNo(cardPaymentRequestDTO.cardNo());
        ...
        if(account.getBalance().compareTo(cardPaymentRequestDTO.amount())>0){
